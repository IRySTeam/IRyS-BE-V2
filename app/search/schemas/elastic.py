...
class ElasticSearchDocumentItem(BaseModel):
    query: str = Field(
        ...,
        description="The query that was evaluated against the document to produce relevance score",
    )
    ner_entity: List[str] = Field(
        ..., description="Key value pair mappings of recognized NER extracted entities"
    )
    metadata_entity: List[str] = Field(
        ...,
        description="Key value pair mappings of recognized document metadata entities",
    )
    score: float = Field(
        ...,
...
class ElasticSearchResult(BaseModel):
    result: List[ElasticSearchDocumentItem] = Field(
        ..., description="Collection of matched documents from ElasticSearch"
    )
